...
// src/view.ts
var SlideAndRevealView = class _SlideAndRevealView extends import_obsidian2.ItemView {
  constructor(leaf, plugin) {
    ...
    this.plugin = plugin;
    this.escScope = new import_obsidian2.Scope(this.app.scope);
  ...
  async onOpen() {
    ...
    const refresh = (f) => {
      if (f instanceof import_obsidian2.TFile && IMG_RE.test(f.path)) this.render();
    };
    ...
    this.registerDomEvent(this.containerEl, "keydown", (e) => {
      ...
      if (!inField && (e.key === "Delete" || e.key === "Backspace") && this.selection) {
        e.preventDefault();
        e.stopPropagation();
  ...
  async loadFolderData() {
    ...
    try {
      ...
      new import_obsidian2.Notice(`Slide & Reveal: couldn't read ${path}`);
    }
  }
  async saveFolderData() {
    ...
    try {
      ...
      new import_obsidian2.Notice("Slide & Reveal: save failed (see console)");
    }
  }
  scheduleSave() {
  ...
  async applyOp(op, opposite) {
    if (op.type === "data") {
      ...
      if (!(file instanceof import_obsidian2.TFile)) {
        new import_obsidian2.Notice(`Can't undo rename: file not found at ${cur}`);
        return;
      }
      ...
      try {
        ...
        console.error(e);
        new import_obsidian2.Notice("Rename undo failed (see console)");
        this[opposite].pop();
      }
    }
  }
  async undo() {
    ...
    if (!op) {
      new import_obsidian2.Notice("Nothing to undo");
      return;
    }
  ...
  async redo() {
    ...
    if (!op) {
      new import_obsidian2.Notice("Nothing to redo");
      return;
    }
  ...
  render() {
    ...
    (0, import_obsidian2.setIcon)(refreshBtn, "rotate-cw");
    refreshBtn.title = "Refresh view";
    refreshBtn.onclick = () => this.render();
    const resetSlidersBtn = row.createEl("button");
    (0, import_obsidian2.setIcon)(resetSlidersBtn, "list-restart");
    ...
    const updateModeBtn = () => {
      ...
      modeBtn.empty();
      (0, import_obsidian2.setIcon)(modeBtn, m === "study" ? "book-open" : "pencil");
    ...
    (0, import_obsidian2.setIcon)(sizeIcon, "maximize-2");
    ...
    if (!(tf instanceof import_obsidian2.TFolder)) {
    ...
    const walk = (fld) => {
      for (const ch of fld.children) {
        if (ch instanceof import_obsidian2.TFolder) walk(ch);
        else if (ch instanceof import_obsidian2.TFile && IMG_RE.test(ch.path)) images.push(ch);
      }
    };
    walk(tf);
  ...
  renderImage(file) {
    ...
    (0, import_obsidian2.setIcon)(renameBtn, "pencil");
    ...
    const { list } = this.rectsFor(file);
    for (const r of list) this.renderShape(canvas, file, r);
  ...
  addPolyPoint(canvas, file, block, e) {
    ...
    if (!this.polyDraft || this.polyDraft.canvas !== canvas) {
      ...
      this.polyDraft = { file, canvas, block, points: [], svg, poly, cleanup: () => svg.remove() };
    }
  ...
  async commitPolyDraft() {
    ...
    if (draft.points.length < 3) {
      new import_obsidian2.Notice("Need at least 3 points for a polygon.");
      return;
    }
    const { file, points } = draft;
  ...
  renderRail(host, file, canvas) {
    ...
    (0, import_obsidian2.setIcon)(upBtn, "chevron-up");
    ...
    (0, import_obsidian2.setIcon)(downBtn, "chevron-down");
    ...
    for (let i = 0; i < rect.points.length; i++) {
      ...
      v.addEventListener("contextmenu", async (e) => {
        ...
        if (!rect.points || rect.points.length <= 3) {
          new import_obsidian2.Notice("A polygon needs at least 3 points.");
          return;
        }
        this.snapshot();
  ...
  refreshHeaderTools() {
    ...
    if (file && this.polyDrawingPaths.has(file.path)) {
      const doneBtn = tools.createEl("button", { cls: "sNr-iconbtn sNr-iconbtn-icon-only sNr-poly-done" });
      (0, import_obsidian2.setIcon)(doneBtn, "check");
      doneBtn.title = "Finalize polygon";
      ...
      (0, import_obsidian2.setIcon)(cancelBtn, "x");
      cancelBtn.title = "Cancel polygon";
      cancelBtn.onclick = () => {
        this.cancelPolyDraft();
        if (file) this.polyDrawingPaths.delete(file.path);
        this.render();
      };
    }
  ...
  currentImageContext() {
    ...
    if (!(f instanceof import_obsidian2.TFile)) return null;
  ...
  iconBtn(parent, icon, text, opts) {
    ...
    (0, import_obsidian2.setIcon)(ico, icon);
  ...
  attachStepperButtons(wrap, input) {
    ...
    (0, import_obsidian2.setIcon)(up, "chevron-up");
    up.title = "Increase";
    up.onclick = (e) => {
      e.stopPropagation();
      input.stepUp();
    ...
    (0, import_obsidian2.setIcon)(down, "chevron-down");
    down.title = "Decrease";
    down.onclick = (e) => {
      e.stopPropagation();
      input.stepDown();
  ...
  selectShape(canvas, file, rect, el) {
    ...
    const setToggleIcon = (revealed) => {
      bToggle.empty();
      (0, import_obsidian2.setIcon)(bToggle, revealed ? "eye" : "eye-off");
    ...
    const setFlashIcon = (icon) => {
      bFlash.empty();
      (0, import_obsidian2.setIcon)(bFlash, icon);
    };
    const refreshFlash = () => {
    ...
    (0, import_obsidian2.setIcon)(insertBtn, "list-plus");
    ...
    insertBtn.onclick = async (e) => {
      ...
      if (!isFinite(target) || target <= 0) {
        new import_obsidian2.Notice("Enter a positive pair number first.");
        return;
      }
      const { list } = this.rectsFor(file);
      ...
      if (!willChange) {
        new import_obsidian2.Notice("Pair numbers above " + target + " are already compact.");
        return;
      }
      this.snapshot();
      let changed = 0;
      ...
      await this.saveFolderData();
      new import_obsidian2.Notice(`Renumbered ${changed} shape${changed === 1 ? "" : "s"} above pair ${target}.`);
      this.render();
    };
    tb.createDiv({ cls: "sNr-tb-divider" });
  ...
  renameFile(file) {
    new RenameModal(this.app, file.name, async (newName) => {
      ...
      try {
        ...
        console.error(e);
        new import_obsidian2.Notice("Rename failed: " + (e instanceof Error ? e.message : String(e)));
      }
    }).open();
  }
};
...
var SlideAndRevealSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName("Mode").setDesc("Study: scrolling inside an image steps the reveal slider. Edit: wheel scrolls normally \u2014 use this when you're drawing/arranging shapes. Toggle from the header any time.").addDropdown((d) => d.addOption("study", "Study").addOption("edit", "Edit").setValue(this.plugin.settings.mode).onChange(async (v) => {
    ...
    new import_obsidian3.Setting(containerEl).setName("Default reveal time (seconds)").setDesc("Used by the \u23F1 button when a rectangle has no per-rect override.").addText((t) => t.setValue(String(this.plugin.settings.defaultSeconds)).onChange(async (v) => {
      const n = parseFloat(v);
      this.plugin.settings.defaultSeconds = isFinite(n) && n > 0 ? n : 3;
      await this.plugin.saveSettings();
    }));
    new import_obsidian3.Setting(containerEl).setName("Default rectangle color").addColorPicker((c) => c.setValue(this.plugin.settings.defaultColor).onChange(async (v) => {
      this.plugin.settings.defaultColor = v;
      await this.plugin.saveSettings();
    }));
    new import_obsidian3.Setting(containerEl).setName("Reveal slider position").setDesc("Which side of each image the reveal/hide rail sits on.").addDropdown((d) => d.addOption("left", "Left").addOption("right", "Right").setValue(this.plugin.settings.railSide).onChange(async (v) => {
    ...
    new import_obsidian3.Setting(containerEl).setName("Left/Right arrows zoom in/out").setDesc("When the view is focused, \u2190 shrinks and \u2192 grows by the step below.").addToggle((t) => t.setValue(this.plugin.settings.arrowKeysZoom).onChange(async (v) => {
      this.plugin.settings.arrowKeysZoom = v;
      await this.plugin.saveSettings();
    }));
    new import_obsidian3.Setting(containerEl).setName("Arrow-key zoom step (%)").setDesc("How many percentage points each \u2190/\u2192 press changes the size.").addText((t) => t.setValue(String(this.plugin.settings.arrowKeysZoomStep)).onChange(async (v) => {
      const n = parseInt(v, 10);
      this.plugin.settings.arrowKeysZoomStep = isFinite(n) && n > 0 ? n : 5;
      await this.plugin.saveSettings();
    }));
    new import_obsidian3.Setting(containerEl).setName("Up/Down arrows reveal/hide on the focused image").setDesc("Independent of mode and zoom: when ON, \u2191 and \u2193 always step the reveal slider on the focused image. When OFF, \u2191/\u2193 pass through to native page scroll (so you can scroll the image list with the keyboard instead).").addToggle((t) => t.setValue(this.plugin.settings.arrowKeysReveal).onChange(async (v) => {
      this.plugin.settings.arrowKeysReveal = v;
      await this.plugin.saveSettings();
    }));
    new import_obsidian3.Setting(containerEl).setName("Invert reveal arrow direction").setDesc("Off (default): \u2191 hides, \u2193 reveals. On: \u2191 reveals, \u2193 hides.").addToggle((t) => t.setValue(this.plugin.settings.arrowKeysRevealInverted).onChange(async (v) => {
      this.plugin.settings.arrowKeysRevealInverted = v;
      await this.plugin.saveSettings();
    }));
    new import_obsidian3.Setting(containerEl).setName("Wheel steps reveal even when zoomed past 100%").setDesc("By default, scrolling on an image whose zoom is over 100% scrolls the page (so the wider image is reachable). Turn this on to keep the wheel locked to the reveal slider regardless of zoom. Only applies in Study mode.").addToggle((t) => t.setValue(this.plugin.settings.wheelStepPast100).onChange(async (v) => {
      this.plugin.settings.wheelStepPast100 = v;
      await this.plugin.saveSettings();
    }));
    new import_obsidian3.Setting(containerEl).setName("Mouse-wheel sensitivity (px per step)").setDesc("Pixels of wheel-delta needed to advance the reveal slider one step. Bump this up if a single click of your mouse wheel jumps too many steps (common with gaming mice on Windows). Default 60.").addText((t) => t.setValue(String(this.plugin.settings.wheelStepThreshold)).onChange(async (v) => {
      const n = parseInt(v, 10);
      this.plugin.settings.wheelStepThreshold = isFinite(n) && n >= 10 ? n : 60;
      await this.plugin.saveSettings();
    }));
    new import_obsidian3.Setting(containerEl).setName("Pair color propagation").setDesc("How a color change on one member of a pair affects the others.").addDropdown((d) => d.addOption("off", "Off \u2014 each shape keeps its own color").addOption("first-to-rest", "First \u2192 rest (only the pair leader propagates)").addOption("all", "All \u2014 every change propagates to the whole pair").setValue(this.plugin.settings.pairColorMode).onChange(async (v) => {
    ...
    if (!known.length) {
      ...
      for (const folder of known) {
        const s = new import_obsidian3.Setting(containerEl).setName(folder);
        ...
        s.addButton((b) => b.setButtonText("Delete annotations file").setWarning().onClick(async () => {
          ...
          try {
            if (await this.app.vault.adapter.exists(path)) {
              ...
              new import_obsidian3.Notice(`Deleted ${path}`);
            }
          } catch (e) {
            console.error(e);
            new import_obsidian3.Notice("Delete failed (see console)");
          }
          await this.plugin.forgetFolder(folder);
          this.display();
        }));
    ...
    this.scanDiscovered().then((discovered) => {
      ...
      for (const folder of fresh) {
        const s = new import_obsidian3.Setting(discoveredEl).setName(folder);
...
// src/main.ts
var SlideAndRevealPlugin = class extends import_obsidian4.Plugin {
  async onload() {
    ...
    this.addRibbonIcon("image", "Open Slide & Reveal", () => {
      ...
      else new import_obsidian4.Notice('Right-click a folder in the file explorer and choose "Open Slide & Reveal here".');
    });
    this.addCommand({
      ...
      callback: () => {
        const last = this.settings.knownFolders[this.settings.knownFolders.length - 1];
        if (last) this.openForFolder(last);
        else new import_obsidian4.Notice("No folder yet. Right-click a folder to open it here.");
      }
    });
    this.addCommand({
      ...
      callback: () => {
        ...
        if (!folders.length) {
          new import_obsidian4.Notice('No folders yet. Right-click a folder and choose "Open Slide & Reveal here" first.');
          return;
        }
